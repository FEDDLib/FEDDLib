...
NonLinearProblem<SC,LO,GO,NO>::NonLinearProblem(CommConstPtr_Type comm):
...
template<class SC,class LO,class GO,class NO>
NonLinearProblem<SC,LO,GO,NO>::NonLinearProblem(ParameterListPtr_Type &parameterList, CommConstPtr_Type comm):
...
void NonLinearProblem<SC,LO,GO,NO>::initVectorSpaces( ){
    ...
    else if( !type.compare("Teko") || type == "FaCSI" || type == "FaCSI-Teko" )
