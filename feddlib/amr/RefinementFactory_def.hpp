...
void RefinementFactory<SC,LO,GO,NO>::refineMesh( MeshUnstrPtr_Type meshP1, int iteration, MeshUnstrPtr_Type outputMesh){
	...
	*this->bcFlagRep_ = *meshP1->bcFlagRep_;
	...
	if( iteration ==0 ){
		this->assignEdgeFlags(meshP1,edgeElements);
	}
	...
	if (this->dim_ == 2 || this->dim_ == 3){
		...
		this->buildTriangleMap();
		MESH_TIMER_STOP(surfaceMapTimer);*/	
		...
		/*for(int i=0; i< this->surfaceTriangleElements_->numberElements() ;i++){
			...
			cout << " und liegt an Element local " << elementsOfSurfaceLocal[i][0] ;
			if(elementsOfSurfaceLocal[i].size()>1)
				cout << " " << elementsOfSurfaceLocal[i][1] ;
			cout << endl;
		}*/
...
vec_bool_Type RefinementFactory<SC,LO,GO,NO>::checkInterfaceSurface( EdgeElementsPtr_Type edgeElements,vec_int_Type originFlag, vec_int_Type edgeNumbers, int indexElement){
		...
		if(edgeElements->getElement(edgeNumbers[1]).isInterfaceElement() && edgeElements->getElement(edgeNumbers[2]).isInterfaceElement() && edgeElements->getElement(edgeNumbers[5]).isInterfaceElement() && (originFlag[2] == 10))
					interfaceSurface[2] = true;
		if(edgeElements->getElement(edgeNumbers[3]).isInterfaceElement() && edgeElements->getElement(edgeNumbers[4]).isInterfaceElement() && edgeElements->getElement(edgeNumbers[5]).isInterfaceElement() && (originFlag[3] == 10 ))
...
void RefinementFactory<SC,LO,GO,NO>::refinementRestrictions(MeshUnstrPtr_Type meshP1, ElementsPtr_Type elements ,EdgeElementsPtr_Type edgeElements, int iteration, int& newPoints, int& newPointsCommon, vec_GO_Type& globalInterfaceIDsTagged, MapConstPtr_Type mapInterfaceEdges, string restriction, int& newElements){
	...
	if(this->dim_ == 2){
		...
		int greenGen=3;//Indicator for our greenCheckSearch-> as we add Points for blue refinement, new to be green refined elements might accur-> nextGenGreen Elements-> how many Generations do we check?
		int entry;
		...
		while(layer >0){
			...
			layer=0;
			if(refinementRestriction_ == "keepRegularity"){
				for(int i=0;i<elements->numberElements() ;i++){
			...
			else if (refinementRestriction_ == "checkGreenTags"){
				for(int i=0;i<elements->numberElements() ;i++){
		...
		for(int i=0;i<elements->numberElements() ;i++){
			if(elements->getElement(i).isTaggedForRefinement()==false){ // only looking at the untagged Elements
				...
				if(tagCounter2[i] == 1){
					...
					this->refineRed(edgeElements, elements, i);
	...
	// Irregular Refinement algorithm according to Bey
		...
		for(int i=0;i<elements->numberElements() ;i++){
			...
			if(elements->getElement(i).isTaggedForRefinement()==false){ // only looking at the untagged Elements
				...
				if(nodeTag >3 && edgeTag >2 ){
					cout << " still requesting regular refinement ? " << endl;
				}
...
int RefinementFactory<SC,LO,GO,NO>::determineLongestEdge( EdgeElementsPtr_Type edgeElements, vec_int_Type edgeVec, vec2D_dbl_ptr_Type points){
	...
	vec_dbl_Type length(this->dim_+1);
...
// We use this Refinement Type 
	if(this->dim_ == 3){ 
		...
		newTrianglesFlag[12]= 10; 
		newTrianglesFlag[13]= 10; 
		newTrianglesFlag[14]= 10; 
