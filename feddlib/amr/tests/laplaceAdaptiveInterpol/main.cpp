...
int main(int argc, char *argv[]) {
    ...
    
    {
		...
		Teuchos::RCP<ExporterParaViewAMR<SC,LO,GO,NO> > exporterSol_ (new ExporterParaViewAMR<SC,LO,GO,NO>());

		BlockMultiVectorConstPtr_Type tmpSol;
		BlockMultiVectorConstPtr_Type firstSol;
		MultiVectorPtrConst_Type FehlerExport;
		while(j<maxIter+1 ){
			...
			if(j>0){
				MultiVectorPtr_Type Fehler = Teuchos::rcp(new MultiVector_Type( domain->getMapUnique() ) ); 
				Fehler->update( 1., tmpSol->getBlock(0), -1. ,laplace->getSolution()->getBlock(0), 0.);
				Fehler->abs(Fehler);
			...
			if(j==1){	
				...
				exporterSol_->setup( "InterpolFehler" , domain->getMesh(),  domain->getFEType(), parameterListAll );
				
				exporterSol_->addVariable(FehlerExport, "InterpolFehler", "Scalar", 1, domain->getMapUnique() );	
		   		exporterSol_->save( (double) j);				
				}
			else if(j>1){
				exporterSol_->reSetup(domain->getMesh());
				exporterSol_->updateVariables(FehlerExport, "InterpolFehler");
		    	exporterSol_->save( (double) j);
			...
			if(j<maxIter)
				tmpSol = meshRefiner.initerpolateSolution( domainP1, domainRefined, firstSol, "taggedEdges");
