...
#define ID_NoFirstOrderDerivatives 82 /* == NoParameters*/
#define ID_NoSecondOrderDerivatives 83
#define ID_NoThirdOrderDerivatives 84
...
#define ID_NoSensDerivatives    100  /*NoFirstOrderDerivatives+NoSecondOrderDerivatives+NoThirdOrderDerivatives*/
...
#define IData_Last              103   /* used space in IData field*/
...
typedef struct{
   ...
   /* zero Bt, Bp, dB in the case that there are no boundary conditions specified
    dimension (NoDOF+(1+NoSensDerivatives))NoMultipliers */ 
    void *dummy1;
    void *dummy2; 
    void *dummy3;
    double *DOFScaling;/* DOF scaling factor for multifield problems - NoDOF numbers*/
    int *SensBVFIndex;/* boundary conditions design velocity field index NoDOF*NoSensDerivatives 
...
typedef struct{
    struct{
        ...
        int Active; /* >0 active element for assembly
                    1 - original active element
                    2 - added active element
                    0 - deleted element
                    -1 -original inactive (staggered schemes)
                    -2 -added inactive (staggered schemes)
                    */
    }id;
    double *Data;
...
typedef struct ElementSpec{
    ...
    struct{
        ...
        int NoIntSwitch;
        int NoDoubleSwitch;
        int CreateDummyNodes;  /*unused - for compatibility with old c files only*/
