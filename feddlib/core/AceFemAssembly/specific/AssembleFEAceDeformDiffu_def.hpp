...
AssembleFE<SC,LO,GO,NO>(flag, nodesRefConfig, params, tuple)
{
    ...
    E0_ = this->params_->sublist("Parameter").get("E0",379.95e-6);
    E1_ = this->params_->sublist("Parameter").get("E1",300.0e-6);
    poissonRatio_ = this->params_->sublist("Parameter").get("Poisson Ratio",0.49e-0);
    c1_ = this->params_->sublist("Parameter").get("c1",0.25e-0);
    D0_ = this->params_->sublist("Parameter").get("D0",6.0e-5);
    m_ = this->params_->sublist("Parameter").get("m",0.0);
...
skr_DeformationDiffusionNeoHook(&v[0],&d[0],&ul[0],&ul0[0],&xl[0],&s[0],&p[0],&ht[0],&hp[0],deltat);
