...
template <class LO, class GO, class NO>
Map_Xpetra<LO,GO,NO>::Map_Xpetra( XpetraMapConstPtrConst_Type& xpetraMapPtrIn ):
map_( xpetraMapPtrIn )
{
...
template < class LO, class GO, class NO>
typename Map_Xpetra<LO,GO,NO>::MapPtr_Type Map_Xpetra<LO,GO,NO>::buildVecFieldMap(UN numDofs, std::string ordering) const{
...
template < class LO, class GO, class NO>
typename Map_Xpetra<LO,GO,NO>::XpetraMapConstPtr_Type Map_Xpetra<LO,GO,NO>::getXpetraMap() const{
...
template < class LO, class GO, class NO>
typename Map_Xpetra<LO,GO,NO>::ThyraVSBConstPtr_Type Map_Xpetra<LO,GO,NO>::getThyraVectorSpaceBase() const{
...
template < class LO, class GO, class NO>
GO Map_Xpetra<LO,GO,NO>::getGlobalElement(LO id) const{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
    return map_->getGlobalElement(id);
}

template < class LO, class GO, class NO>
LO Map_Xpetra<LO,GO,NO>::getLocalElement(GO id) const{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
    return map_->getLocalElement(id);
}

template < class LO, class GO, class NO>
LO Map_Xpetra<LO,GO,NO>::getNodeNumElements() const{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
    return map_->getLocalNumElements();
}

template < class LO, class GO, class NO>
GO Map_Xpetra<LO,GO,NO>::getGlobalNumElements() const{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
    return map_->getGlobalNumElements();
}

template < class LO, class GO, class NO>
Teuchos::ArrayView< const GO > Map_Xpetra<LO,GO,NO>::getNodeElementList() const{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
    return map_->getLocalElementList();
}
    
template < class LO, class GO, class NO>
GO Map_Xpetra<LO,GO,NO>::getMaxAllGlobalIndex() const{
    return map_->getMaxAllGlobalIndex();
}
    
template < class LO, class GO, class NO>
LO Map_Xpetra<LO,GO,NO>::getMaxLocalIndex() const{
    return map_->getMaxLocalIndex();
}
    
template < class LO, class GO, class NO>
void Map_Xpetra<LO,GO,NO>::print(Teuchos::EVerbosityLevel verbLevel) const{
...
template < class LO, class GO, class NO>
typename Map_Xpetra<LO,GO,NO>::CommConstPtr_Type Map_Xpetra<LO,GO,NO>::getComm() const{
    
    return map_->getComm();
}

template < class LO, class GO, class NO>
typename Map_Xpetra<LO,GO,NO>::CommPtr_Type Map_Xpetra<LO,GO,NO>::getCommNonConst() {
    CommConstPtr_Type commConst = map_->getComm();
    return Teuchos::rcp_const_cast<Comm_Type>(commConst);
}
    
template <class LO,class GO,class NO>
Teuchos::RCP<Map_Xpetra<LO,GO,NO> > Map_Xpetra<LO,GO,NO>::buildUniqueMap( int numFreeProcs ) const
{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
    if (numFreeProcs==0) {
...
template <class LO,class GO,class NO>
Teuchos::RCP<Map_Xpetra<LO,GO,NO> > Map_Xpetra<LO,GO,NO>::buildUniqueMap( tuple_intint_Type rankRange ) const
{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
...
template <class LO,class GO,class NO>
GO Map_Xpetra<LO,GO,NO>::getIndexBase() const{
    TEUCHOS_TEST_FOR_EXCEPTION(map_.is_null(),std::runtime_error,"map is null.");
