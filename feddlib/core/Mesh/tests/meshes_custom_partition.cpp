...
int main(int argc, char *argv[]) {
    ...
    string xmlFile = "meshes_custom_partition.xml";
    myCLP.setOption("file",&xmlFile,"xmlFile");
    idx_t nparts = comm->getSize();
    myCLP.setOption("nparts",&nparts,"nparts");
    ...
    int dim	= pList->sublist("Parameter").get("Dimension",2);
    string meshType = pList->sublist("Parameter").get("Mesh Type","unstructured");
    string meshName_fluid = pList->sublist("Parameter").get("Mesh Name Fluid","dfg_fsi_fluid_h004.mesh");
    string meshName_struc = pList->sublist("Parameter").get("Mesh Name Structure","dfg_fsi_solid_h004.mesh");
    string meshDelimiter = pList->sublist("Parameter").get("Mesh Delimiter"," ");
    string discType = pList->sublist("Parameter").get("Discretization","P1");
    ...
    int nmbInterfaceFlags = pList->sublist("Parameter").get("Interface Flags",1);
    ...
    
    {
        if (verbose) {
            cout << "#################################################################" <<endl;
            cout << "#### Starting custom partition of meshes with interfaces ... ####" <<endl;
            cout << "#################################################################" <<endl;
        }
...

        {
            Teuchos::TimeMonitor totalTimeMonitor(*totalTime);
            {
                ...
                if (!meshType.compare("unstructured")) {
                    ...
                    vec_int_Type idsInterface(1,5); //interface flag is 5
                    if (dim==3) {
                        idsInterface[0] = 6; // interface flag is 6
                        if (nmbInterfaceFlags==2)
                            idsInterface.push_back(3); // and 3
                    }
                    if (verbose)
                        cout << "\n";
...
//            exPara->setup(domain->getDimension(), domain->getNumElementsGlobal(), domain->getElements(), domain->getPointsUnique(), domain->getMapUnique(), domain->getMapRepeated(), discType, "fluid", 1, comm);
        ...
        if (exportParaViewSubomains) {
            ...
            exPara->setup( "subdomains_fluid_custom", domain->getMesh(), "P0" );
...
//            exPara->setup(domain->getDimension(), domain->getNumElementsGlobal(), domain->getElements(), domain->getPointsUnique(), domain->getMapUnique(), domain->getMapRepeated(), discType, "solid", 1, comm);
        ...
        if (exportParaViewSubomains) {
            ...
            exPara->setup( "subdomains_solid_custom", domain->getMesh(), "P0" );
